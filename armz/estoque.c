...
void editProdDoacao(Cadastro *prod, int totalProd)
{
    int subtrairQtd = 0, confirmacao;
    int index;
    char destinoDoacao[101]; 
    printf("Digite o número do produto que deseja editar (1 a %d): ", totalProd);
    scanf("%d", &index);
    index--;
    ...
    do
    {
        ...
        case 1:
            ...
            do
            {
                ...
                if (subtrairQtd <= 0)
                {
                    printf("Erro: A quantidade a subtrair deve ser maior que zero.\n");
                }
                else if (subtrairQtd > editProd->qtd)
                {
                    printf("Erro: A quantidade a subtrair é maior que a quantidade disponível.\n");
                }
            } while (subtrairQtd <= 0 || subtrairQtd > editProd->qtd);

            // Subtrai a quantidade
            editProd->qtd -= subtrairQtd;
            printf("Nova quantidade: %d\n", editProd->qtd);
            printf("Deseja confirmar a doação? (1 - Sim / 0 - Não): ");
            scanf("%d", &confirmacao);
            if (confirmacao == 1)
            {
                // Subtrai a quantidade
                editProd->qtd -= subtrairQtd;
                printf("Qual instituição será doada?: ");
                scanf("%s", destinoDoacao);
                printf("Quantidade doada: %d\n", editProd->qtd);
                printf("Nome Instituição: %s\n", destinoDoacao);

            }
            else
            {
                // Desfaz a operação
                printf("Alteração cancelada. Quantidade permanece: %d\n", editProd->qtd);
            }
            break;
        case 0:
            printf("Saindo da edição.\n");
...
void descarteDoacao(Cadastro *prod, int totalProd)
{
    int subtrairQtd = 0, confirmacao;
    int index;
    char destinoDoacao[101]; 
    ...
    do
    {
        ...
        case 1:
            ...
            editProd->qtd -= subtrairQtd;
            printf("Nova quantidade: %d\n", editProd->qtd);
            printf("Deseja confirmar o Descarte? (1 - Sim / 0 - Não): ");
            scanf("%d", &confirmacao);
            if (confirmacao == 1)
            {
                // Subtrai a quantidade
                editProd->qtd -= subtrairQtd;
                printf("Quantidade Descartada: %d\n", editProd->qtd);
            }
            else
            {
                // Desfaz a operação
